...
def func_LOWERCASE(arr):
    low_arr = []
    for element in arr:
        element = element.lower()
        low_arr.append(element)
    return low_arr

def func_GETWORDS(lowered_news):
    words = []
    for new in lowered_news:
...
def func_INDEX(words,lowered_news):

    whereisword = []

    for key in words:
        arr = []
        for i in range(len(lowered_news)):
            if lowered_news[i].find(key) != -1:
                arr.append(i)
        whereisword.append(arr)

    return whereisword

def func_OR(request,where_is_word,words):

    request_words = func_LOWERCASE(request.split(" "))
    solution = []
    temp_solution = []

    for request_word in request_words:
        if request_word not in words:
            continue
        temp_solution.extend(where_is_word[words.index(request_word)])
...
def func_AND(request,where_is_word,words):
    solution = []
    request_words = func_LOWERCASE(request.split(" "))
    for request_word in request_words:
        if request_word not in words:
    ...
    solution.extend(where_is_word[words.index(request_words[0])])

    for request_word in request_words:
        for s in solution:
            if s not in where_is_word[words.index(request_word)]:
                solution.remove(s)
...
if __name__ == '__main__':
    ...
    main_lowered_news = func_LOWERCASE(all_news)
    main_words = func_GETWORDS(main_lowered_news)
    main_where_is_word = func_INDEX(main_words,main_lowered_news)

    main_request = ""
    while True:

        main_request = input("Please send request!Write your word that you want to search!('q' for quit!)")

        if main_request.lower() == "q":
            break

        main_choice = input("Choose one AND or OR!Write 'AND' or 'OR'!")
        if main_choice.lower() == "and":
            print(func_AND(main_request,main_where_is_word,main_words))
        elif main_choice.lower() == "or":
            print(func_OR(main_request,main_where_is_word,main_words))
        else:
            print("Over Case!")
